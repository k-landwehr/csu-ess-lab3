...
pop <- pop_dat %>%
  filter(COUNTY !=0) %>%
  mutate(fips = paste0(sprintf("%02s", STNAME),
                       sprintf("%03s", COUNTY))) %>%
...
county_deaths <- colorado %>%
  ...
  mutate(death_ratio = 100 * sum(new_deaths, na.rm = TRUE) / first(DEATHS2021)) %>%
  filter(death_ratio > 20) %>%
  summarise(
    deaths = sum(new_deaths, na.rm = TRUE),  
    DEATHS2021 = first(DEATHS2021),  
    death_ratio = first(death_ratio)) %>%
  filter(year == 2021)
...
pp = pop %>%
  ...
  summarise(colorado = 
              sum(POPESTIMATE2021)) %>%
  inner_join(colorado, by = c("STNAME" = "state")) %>%
  mutate(perCap = newCases / colorado) %>%
  group_by(STNAME) %>%
